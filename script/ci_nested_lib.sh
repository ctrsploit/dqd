#!/usr/bin/env bash
# Shared helpers for nested CI builds (building inside a dqd builder VM).
# Sourced by ci_run.sh and ci_cluster.sh. Expects ENV_FILE to be set by the
# caller before invoking env_value / set_ci_dqd_builder.
#
# Defaults may be overridden by the caller via environment variables.
