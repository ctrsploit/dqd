...
- For Kubernetes environments that include `kubeconfig`, do **not** trust the `docker_archive` kubeconfig as the final file. `kubeconfig` contains cluster certificates generated by `kubeadm init`; after CI builds the migrated image, `dqd-verify` must pull `/etc/kubernetes/admin.conf` from the running migrated environment and rewrite its server to the host API port, for example `https://127.0.0.1:<host-6443-port>`.
