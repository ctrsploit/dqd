...
check_all_pods_ready() {
  kubectl wait --for=condition=Ready pod --all -A --timeout=5s >/dev/kmsg 2>&1
}
...
# Fix Calico: path /sys/fs is mounted on /sys but it is not a shared mount
ensure_shared_root_mount() {
  log "Ensuring root mount is shared to resolve Calico mount issues"
  mount --make-rshared /
}
...
install_calico_network() {
  ...
  helm install calico projectcalico/tigera-operator --version v3.30.6 --namespace tigera-operator >>/dev/kmsg 2>&1
